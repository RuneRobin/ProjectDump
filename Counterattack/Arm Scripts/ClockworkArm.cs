...
public class ClockworkArm : ArmStats
{
    ...
    void Update()
    {
        ...
        if (CharacterLoader.instance.currentlyFighting == true)
        {
            ...
            if (windUpTimer < 0 || player.timesArmsHaveWoundUp > 0)
            {
                windUpTimer = 0;
            }
            ...
            if (isAttacking == false && windUpTimer <= 0)
            {
                ...
                isAttacking = true;
                if (player.timesArmsHaveWoundUp > 0)
                {
    ...
    public void OnTriggerEnter2D(Collider2D collision)
    {
        ...
        {
            collision.gameObject.GetComponent<EnemyStats>().health -= damage + bonusFistDamage + bonusDamage;
            isAttacking = false;
