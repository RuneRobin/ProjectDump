...
public class TantrumArm : ArmStats
{
    ...
    public int tantrumStacks;
    private int maxTantrum;
    ...
    void Update()
    {
        ...
        if (CharacterLoader.instance.currentlyFighting == true)
        {
            ...
            if (Input.GetMouseButton(0) && isAttacking == false && tantrumStacks >= maxTantrum && cooldownTimer >= 1 - bonusCooldown)
            {
                isAttacking = true;
                tantrumStacks -= maxTantrum;
                Attack();
            }
        }
        ...
        if(CharacterLoader.instance.currentlyFighting == false && tantrumStacks > 0)
        {
            tantrumStacks = 0;
        }
    }
