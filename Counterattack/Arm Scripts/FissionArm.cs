...
public class FissionArm : ArmStats
{
    ...
    void Update()
    {
        ...
        if (CharacterLoader.instance.currentlyFighting == true)
        {
            ...
            if (Input.GetMouseButton(0) && isAttacking == false && exploded == false && cooldownTimer >= 1 - bonusCooldown)
            {
                isAttacking = true;
                Attack();
            }
        ...
        if(CharacterLoader.instance.currentlyFighting == false && exploded == true)
        {
            exploded = false;
        }
    }
    ...
    public void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.tag == "Enemy" && collision.GetComponent<EnemyStats>().openToAttack == true)
        {
            ...
            detonationChance++;
            if(detonationChance*level > Random.Range(0,101))
            {
                enemy.health -= damage;
