...
public class OverclockedArm : ArmStats
{
    ...
    void Update()
    {
        ...
        if (CharacterLoader.instance.currentlyFighting == true)
        {
            ...
            if (Input.GetMouseButtonDown(0) && isAttacking == false && windUpTimer <= 0 && cooldownTimer >= 1 - bonusCooldown)
            {
                isAttacking = true;
    ...
    public void OnTriggerEnter2D(Collider2D collision)
    {
        if (collision.tag == "Enemy" && collision.GetComponent<EnemyStats>().openToAttack == true)
        {
            ...
            windUpTimer = windUp - (level * windUpReduction) - bonusWindUpReduction;
            windUpReduction++;
        }
    }
