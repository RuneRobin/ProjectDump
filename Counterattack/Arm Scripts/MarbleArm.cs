...
public class MarbleArm : ArmStats
{
    ...
    void Start()
    {
        ...
        bulletCount = 1;
        bulletPrefab = Resources.Load("Sprites/Items/DeadeyeBullet") as GameObject;
        firerate = 0.1f;
    ...
    public IEnumerator Attack()
    {
        ...
        Vector3 direction = armToTargetDiff * speed; //so that when each bullet is being fired it doesn't update to wherever the mouse is but instead they all fire towards the first shot's location
        gameObject.transform.rotation = Quaternion.Euler(Vector3.forward * diff);

        int extraMarbles = 0;
        for (int i = 0; i < level; i++)
        {
            if(marbleChance > Random.Range(0,101))
            {
                extraMarbles++;
            }
        }

        for (int i = 0; i < bulletCount + extraMarbles + bonusBulletCount; i++)
        {
            bullet = Instantiate(bulletPrefab, transform.position, transform.rotation);
            bullet.GetComponent<BulletHandler>().bulletDamage = damage + bonusDamage;
            bullet.GetComponent<Rigidbody2D>().velocity = direction;
            bullet.transform.rotation = Quaternion.Euler(Vector3.forward * diff);
